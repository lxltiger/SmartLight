...
public abstract class LightService extends Service implements
    ...
    /**
     * 向设备发送控制命令
     * <p>使用{@link android.bluetooth.BluetoothGattCharacteristic#WRITE_TYPE_DEFAULT}方式发送.
     * <p><strong>示例.</strong>
     * <pre>
     * {@code
     * LightService service = ...
     * service.sendCommand((byte) 0xD0, 0xFFFF, new byte[]{0x01, 0x00, 0x00});
     * }
     * </pre>
     *
     * @param opcode  操作码
    ...
    public boolean sendCommand(byte opcode, int address, byte[] params) {
        return this.sendCommand(opcode, address, params, null, 0, false);
    }

    public boolean sendCommandImmediate(byte opcode, int address, byte[] params) {
        return this.sendCommand(opcode, address, params, null, 0, true);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params, boolean immediate) {
        return this.sendCommand(opcode, address, params, null, 0, immediate);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params, Object tag, boolean immediate) {
        return this.sendCommand(opcode, address, params, tag, 0, immediate);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params, int delay, boolean immediate) {
        return this.sendCommand(opcode, address, params, null, delay, immediate);
    }

    public boolean sendCommand(byte opcode, int address, byte[] params, Object tag, int delay, boolean immediate) {
